...
export function ShareButtons() {
  ...
  const shareText = "Check out Guru Kottureshwara Shop for quality Freedom Oil!";
    ...
    <div className="flex justify-center space-x-4">
        <Button variant="ghost" size="icon" asChild>
            <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" aria-label="Share on WhatsApp">
                <WhatsAppIcon className="h-6 w-6 text-white transition-colors duration-300 hover:text-green-500" />
            </a>
        </Button>
        <Button variant="ghost" size="icon" asChild>
            <a href={facebookUrl} target="_blank" rel="noopener noreferrer" aria-label="Share on Facebook">
                <FacebookIcon className="h-6 w-6 text-white transition-colors duration-300 hover:text-blue-600" />
            </a>
        </Button>
    </div>
  );
